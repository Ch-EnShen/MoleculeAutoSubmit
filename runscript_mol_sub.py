...
if calc_type == "opt-freq":
    ...
    parse_mol_sub.write_in_gjf(s0_opt_log, s0_gjf, molecule_name, funtional, calc_type)
elif calc_type == "nacme":
    s1_opt_log = molecule_name + f'-{funtional}-s1-opt-freq.log'
    s1_gjf = molecule_name + f'-{funtional}-s1-opt-freq.gjf'
    parse_mol_sub.write_in_gjf(s1_opt_log, s1_gjf, molecule_name, funtional, calc_type)
else:
